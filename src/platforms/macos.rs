...
use core_foundation::{
    ...
    array::CFArray,
    dictionary::{CFDictionary, CFDictionaryRef, CFDictionaryGetCount},
...
impl MacOSMonitor {
    ...
    fn request_screen_recording_permission() -> bool {
        unsafe {
            if !CGPreflightScreenCaptureAccess() {
                ...
                if granted {
                    println!("Screen recording permission granted (note: a restart may be needed).");
                } else {
    ...
    unsafe fn setup_observers(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        extern fn handle_notification(_this: *mut Object, _cmd: objc::runtime::Sel, notification: *mut Object) {
        ...
        let success: bool = unsafe {
            ...
            extern fn notification_handler(this: &Object, _: Sel, notification: *mut Object) {
                let verbose = unsafe { VERBOSE };
            ...
            unsafe {
                decl.add_method(sel!(observeNotification:), 
                                notification_handler as extern fn(&Object, Sel, *mut Object));
            }
            ...
            let observer: *mut Object = msg_send![Class::get("RustNotificationObserver").unwrap(), new];
...
fn get_active_window_info() -> Result<Option<WindowInfo>, Box<dyn Error>> {
    unsafe {
        ...
        let window_array: CFArray<CFDictionary> = CFArray::wrap_under_get_rule(window_list as *const _);
        ...
        for i in 0..count {
            ...
            let info_dict: CFDictionary<*const c_void, *const c_void> = unsafe { CFDictionary::wrap_under_get_rule(info) };
            ...
            if owner_name_str == app_name_str {
                ...
                if !window_name_ref.is_null() {
                    // Convert the value to a CFString
                    let window_name = unsafe { CFString::wrap_under_get_rule(window_name_ref as *const _) };
